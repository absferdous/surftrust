...
(function () {
  ...
  function sendHeartbeat() {
    fetch(`${apiUrl}/heartbeat`, {
      method: "POST"
    }).catch(error => console.error("Surftrust: Heartbeat failed.", error));
  }
  ...
  async function startNotificationLoop() {
    ...
    for (const campaign of notificationQueue) {
      ...
      if (maxDisplays > 0 && sessionViewCount >= maxDisplays) {
        console.log(`Surftrust: Frequency cap of ${maxDisplays} reached. Halting loop.`);
        break;
      }
  ...
  function applyStyles(el, campaignSettings) {
    ...
    const positionKey = Object.keys(campaignSettings).find(k => k.includes("notification") || k.includes("alert") || k.includes("displays"));
    const position = campaignSettings[positionKey]?.position || globalCustomize.position || "bottom-left";
